...
Matrix::Matrix(const Matrix &a)
{
	...
	for(int i = 0; i < rowsNum; i++)
	{
		for(int j = 0; j < colsNum; j++)
		{
			matrixData[i][j] = a.matrixData[i][j];
		}
	}
}
...
Matrix& Matrix::operator=(const Matrix &a)
{
	if(this != &a)
	{
		this->rowsNum = a.rowsNum;
		this->colsNum = a.colsNum;

		this->matrixData = new int*[rowsNum];

		for(int i = 0; i < this->rowsNum; i++)
		{
			matrixData[i] = new int[this->colsNum];
		}
...
Matrix Matrix::copy(){
	...
	/* UNCOMMENT THIS SECTION, THEN DEBUG! */	
	Matrix copy(this->rowsNum, this->colsNum);
...
Matrix Matrix::subMatrix(int i, int j){
	...
	Matrix subMatrix(i + 1, j + 1);

	for(int row = 0; row <= i; row++)
	{
		for(int col = 0; col <= j; col++)
		{
			subMatrix.matrixData[row][col] = this->matrixData[row][col];
		}
	}
